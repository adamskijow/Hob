...
CASES = [
    ...
    Case("Bro jack shit got done today", zero_recap,
         ...
         presented=ACTIVE[:2], presented_kind="eod"),
    Case("Jack shit bud", zero_recap,
         "terse slang zero report cannot become a destructive task edit",
         presented=ACTIVE[:2], presented_kind="eod"),
    Case("drop the second one",
         lambda p: kinds(p) == ["drop"] and p.mutations[0].target == "a2",
         "explicit task drop still works during an evening recap",
         presented=ACTIVE[:2], presented_kind="eod"),
    Case("thanks hob",
    ...
    Case("new report is waiting on Sam",
         lambda p: kinds(p) == ["capture"] and p.mutations[0].waiting,
         "new blocked work remains a waiting capture",
